...
from tests.ro_crates_v1_3 import MetadataDocument, MetadataDocumentFormat
    ...
    do_entity_test(
        ...
        expected_triggered_issues=[
            (
                ...
                'does not reference the required context "https://w3id.org/ro/crate/1.3/context"'
            )
        ],
    )
...
def test_singleton_entity_property_is_recommended_as_a_single_value():
    """
    Test the RO-Crate 1.3 recommendation that a singleton property array is
...
def test_schema_org_protocol_shapes_run_after_context_check():
    ...
    assert any(
        issue.message
        and 'does not reference the required context "https://w3id.org/ro/crate/1.3/context"' in issue.message
        for issue in result.get_issues()
    )
    assert not any(
