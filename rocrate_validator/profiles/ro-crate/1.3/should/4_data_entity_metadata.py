...
class WebDataEntityRequiredChecker(PyFunctionCheck):
    ...
    def _not_downloadable_message(entity_id: str, dl) -> str:
        """Build the warning message for a web-based Data Entity that is not directly downloadable."""
                ...
                if content_size:
                    ...
                    if external_size is not None and content_int is not None and content_int != external_size:
                        context.result.add_issue(
                            ...
                            f"the downloadable content, i.e., {external_size} (bytes)",
                            self,
        ...
        for entity in context.ro_crate.metadata.get_web_data_entities():
            ...
            for url in urls:
                ...
                try:
                    ...
                    if not dl.is_downloadable:
                        entity_ref = f"contentUrl '{url_value}' for Web-based Data Entity '{entity.id}'"
                        msg = f"{entity_ref} is not directly downloadable"
                        if dl.reason:
                            msg += f": {dl.reason}"
