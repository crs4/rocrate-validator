...
class RootDataEntityRelativeURI(PyFunctionCheck):
    """
    The Root Data Entity SHOULD be denoted by the string ./ or an absolute URI
    """

    @check(name="Root Data Entity: RECOMMENDED value")
    def check_relative_uris(self, context: ValidationContext) -> bool:
        """Check if the Root Data Entity is denoted by the string `./` or an absolute URI"""
        try:
            ...
            context.result.add_issue(
                'Root Data Entity URI is not denoted by the string `./` or an absolute URI', self)
            return False
