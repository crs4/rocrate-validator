...
@requirement(name="Workflow diagram existence")
class WorkflowDiagramExistence(PyFunctionCheck):
    """A Workflow diagram MAY be present in the RO-Crate."""
    ...
    @check(name="Workflow existence")
    def check_workflow(self, validation_context: ValidationContext) -> bool:
        """Check if the crate contains the workflow file."""
        ...
        if not workflow_path.is_file():
            validation_context.result.add_error(f"{workflow_path} not found in crate", self)
            return False
        return True
    ...
    def check_workflow_diagram(self, validation_context: ValidationContext) -> bool:
        ...
        if not diagram_path.is_file():
            validation_context.result.add_error(f"{diagram_path} not found in crate", self)
            return False
        return True
    ...
    def check_workflow_description(self, validation_context: ValidationContext) -> bool:
        ...
        if not description_relpath:
            validation_context.result.add_error(f"main workflow does not have a 'subjectOf' property", self)
            return False
        description_path = validation_context.rocrate_path / description_relpath
        if not description_path.is_file():
            validation_context.result.add_error(f"{description_path} not found in crate", self)
            return False
        return True
