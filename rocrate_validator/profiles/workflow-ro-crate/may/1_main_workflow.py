    ...
    def check_workflow_diagram(self, context: ValidationContext) -> bool:
        ...
        try:
            ...
            if not diagram_relpath:
                context.result.add_check_issue("main workflow does not have an 'image' property", self)
                return False
            if not image.is_available():
                context.result.add_check_issue(f"Workflow diagram '{image.id}' not found in crate", self)
                return False
            return True
    ...
    def check_workflow_description(self, context: ValidationContext) -> bool:
        ...
        try:
            ...
            if not description_relpath:
                context.result.add_check_issue("main workflow does not have a 'subjectOf' property", self)
                return False
