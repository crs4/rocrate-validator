...
from .checks import CheckIssue, IssueSeverity
...
class ValidationResult:
    ...
    def get_issues_by_severity(self, severity: IssueSeverity) -> List[CheckIssue]:
        return [issue for issue in self._issues if issue.severity == severity]

    def has_issues(self, severity: IssueSeverity = IssueSeverity.WARNING) -> bool:
        return any(issue.severity.value >= severity.value for issue in self._issues)

    def passed(self, severity: IssueSeverity = IssueSeverity.WARNING) -> bool:
